...
contract MyToken is ERC20, Ownable {
    ...
    // Function to redeem an item, burning a specified amount of tokens
    function redeemItem(uint256 amount) public {
        // Burn the specified amount of tokens from the sender
        _burn(msg.sender, amount);
        // Emit an event or handle redemption logic here
        emit ItemRedeemed(msg.sender, amount);
    }

    // Event to log item redemptions
    event ItemRedeemed(address indexed redeemer, uint256 amount);
